...
stream = agent.create_turn(
    messages=[{"role": "user", "content": "What is Artificial Intelligence?"}], session_id=s_id, stream=True
)
for event in stream:
    pprint(event)
...
stream = agent.create_turn(
    messages=[{"role": "user", "content": "What are the prictical applications of AI?"}], session_id=s_id, stream=True
)
